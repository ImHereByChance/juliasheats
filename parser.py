...
Fieldstuple = namedtuple(
    'Fields', 'varieties custumers numbers pieces \
...
def find_main_data(book, sheet: str):
    ...
    for cell in page['A']:
        if cell.value in ('date', 'Ship\ndate'):
            ...
            # check of row follows next to data_beginng row
            if isinstance(page[f'A{data_beginning_row}'].value, float):
                continue
            else:
                raise ValueError(
                    "Error: 'date'-row is found ",
                    "but float-type date-cell don't follow further")
...
def find_quantity_columns(book, sheet: str, mainData_range):
    """
    Finds desposition of columns: numbers, pieces, totals, prices,
    amounts, codes.
    ...
    if beginning is None:
        raise ValueError(
            "couldn't find 'Number' 'Pieces' 'Total' 'Price' ",
            "'Amount' columns layout on the {sheet}")
    ...
    for cell in page['A']:
        if cell.value == section:
            if not page[f'A{cell.row+1}'].value == 'Date':
                raise ValueError(
                    f'error during searching {section}range: \
...
def find_rates_singleUse(book, sheet: str, singleUse_range):
    """Finds disposition of column "Rate" on the page
    and returns it's column-letter. """
    page = book.get_sheet_by_name(sheet)
    ...
    raise ValueError(
        f'could not find disposition of column "Rate" ',
        '(section "Single use packaging") on {sheet}')
...
def find_quantities_multiUse(book, sheet: str, multiUse_range):
    ...
    if page[f'A{data_beginning_row-1}'].value == 'Date':
        ...
        raise ValueError(
            f"couldn't find headline_row of 'Multi use packaging' on {sheet}")

    quantity_columns = [
        cell.column for cell in page[headline_row]
        if cell.value in
        ('Number', 'Deposit', 'Packaging\nrental charge', 'Packaging')]
    if not len(quantity_columns) == 3:
        raise ValueError(
            "couldn't find one of: 'Number', 'Deposit' or 'Packaging rental ",
            f"charge' columns in 'Multi use packaging'-section on {sheet}")
    return tuple(get_column_letter(i) for i in quantity_columns)


def is_gap_after_date(row):
    """
    checks is row is an empty space after 'Date'-row in additional section or
    not(some pages can contain such rows after conversion)
...
def get_range_from_column(book, sheet: str, col_range: tuple, column_name: str):
    ...
    return [
        i[0].value for i
        in page[f'{column_name}{col_range[0]}':f'{column_name}{col_range[1]}']
    ]
...
def check_varieties_or_custumers(data_list: list, book, sheet):
    ...
    for i in data_list:
        if isinstance(i, str) and i[0].isdigit() and ' ' in i:
            ...
            raise ValueError(
                f"'{i}' (position {counter} in column) on page '{sheet} "
                "does not look like valid variety or costumer value'")
        counter += 1
...
def check_codes(codes: list, book, sheet):
    for string in codes:
        ...
        if not len(string) == 3:
            raise ValueError(
                f'code value "{string}" on {sheet} does not look like code')
        for i in string:
            if not i.isdigit():
                raise ValueError(
                    f'code value "{string}" on {sheet} does not look like code')
    ...
    for numb in numbers:
        ...
        for i in numb:
            if not i.isdigit() and i != '.':
                raise ValueError(
                    f'Error during checking Number "{numb}" value ',
                    f'(row {row_numb}) in {column_name} on {sheet}')
...
def check_fractinalStrings(values: list, sheet, column_name):
    ...
    for i in values:
        ...
        if isinstance(res, type(None)):
            raise ValueError(
                f'Error during checking value "{i}" - ',
                f'row {row_numb} in {column_name} on {sheet} ')
...
def correct_priece_format(price: str, book, sheet: str):
    if not isinstance(price, int):
        raise ValueError(
            'error while convert to right format value ',
            f'{price} from column Prise on page "{sheet}"')
    price = str(price)
    if len(price) == 3:
...
def adopt_float_format(rate_value):
    ...
    # if result looks like (1.000.00), remove all excess commas
...
def retrieve_rent_multiUse(book, sheet, data_range, column):
    ...
    retrieved = [i for i in retrieved if i is not None]
    retrieved = split_ifMerged(retrieved)
    check_fractinalStrings(
        retrieved, sheet, 'Packaging rental charge (Multi use)')
    return [adopt_float_format(i) for i in retrieved]
...
def parse(file):
    ...
    all requered columns on the page and merges list of values
    of each retrieved column to general list of the such values.
    E.g., 'varieties += variety' - "variety" is list of variety-values
    from particular sheet in the document whereas "varieties" is list
    of all variety colums from whole document.
    """
    ...
    for sh in sheets:
        ...
        if mainData_range is not None:
            ...
            number = retrieve_number(
                book, sh, mainData_range, quantity_colums[0])
            numbers += number
            piece = retrieve_piece(
                book, sh, mainData_range, quantity_colums[1])
            pieces += piece
            total = retrieve_total(
                book, sh, mainData_range, quantity_colums[2])
            totals += total
            price = retrieve_price(
                book, sh, mainData_range, quantity_colums[3])
            prices += price
            amount = retrieve_amount(
                book, sh, mainData_range, quantity_colums[4])
            amounts += amount
            code = retrieve_code(book, sh, mainData_range, quantity_colums[5])
            codes += code

        singleUse_range = find_additional_section(
            book, sh, 'Single use packaging')
        if singleUse_range is not None:
            ...
            rate_singleUse = retrieve_rate_singleUse(
                book, sh, singleUse_range, rate_disposition)
            rates_singleUse += rate_singleUse

        multiUse_range = find_additional_section(
            book, sh, 'Multi use packaging')
        if multiUse_range is not None:
            quantities_multiUse = find_quantities_multiUse(
                book, sh, multiUse_range)

            code_multiUse = retrieve_code_multiUse(book, sh, multiUse_range)
            codes_multiUse += code_multiUse
            deposit_multiUse = retrieve_deposit_multiUse(
                book, sh, multiUse_range, quantities_multiUse[1])
            deposits_multiUse += deposit_multiUse
            rent_multiUse = retrieve_rent_multiUse(
                book, sh, multiUse_range, quantities_multiUse[2])
